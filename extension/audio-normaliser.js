...
const find_exe_1 = __importDefault(require("./util/find-exe"));
...
class AudioNormaliser {
    ...
    async setup() {
        ...
        const exeMissing = [];
        if (!await (0, find_exe_1.default)('python'))
            exeMissing.push('python');
        if (!await (0, find_exe_1.default)('ffmpeg'))
            exeMissing.push('ffmpeg');
        if (!await (0, find_exe_1.default)('ffmpeg-normalize'))
            ...
            for (const [i, asset] of added.entries()) {
                if (!processing.includes(asset.name)
                    ...
                    try {
                        ...
                        await (0, fs_extra_1.ensureDir)(tempDir); // Ensures temp directory exists
                        await (0, fs_extra_1.copyFile)(original, tempCopy); // Copy to temp
                        await (0, promises_1.unlink)(original); // Delete original
                        ...
                        await exec(cmd);
                        await (0, promises_1.unlink)(tempCopy); // Delete temp
                    }
                    catch (err) {
