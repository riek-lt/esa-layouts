...
import findExecutable from './util/find-exe';

const exec = promisify(execCb);

class AudioNormaliser {
  nodecg!: NodeCGTypes.ServerAPI;
  assets!: NodeCGTypes.ServerReplicantWithSchemaDefault<NodeCGTypes.AssetFile[]>;
  assetsNormalised!: NodeCGTypes.ServerReplicantWithSchemaDefault<NodeCGTypes.AssetFile[]>;

  constructor(nodecg: NodeCGTypes.ServerAPI, assetName = 'videos') {
    this.nodecg = nodecg;
    this.assets = nodecg.Replicant<NodeCGTypes.AssetFile[]>(
      `assets:${assetName}`,
    ) as unknown as NodeCGTypes.ServerReplicantWithSchemaDefault<NodeCGTypes.AssetFile[]>;
    this.assetsNormalised = nodecg.Replicant<NodeCGTypes.AssetFile[]>(
